-- Step 1: Add UNIQUE constraints to the user_id columns in role-specific tables.
-- This is necessary for the ON CONFLICT clause in the trigger to work correctly.
...
-- Step 2: Drop the existing trigger and function to apply the final logic.
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Step 3: Recreate the function with the corrected conditional logic that relies on the new constraints.
...
DECLARE
  ...
  v_subject text;
BEGIN
    -- Insert into public.users, ignoring conflicts on the primary key (id).
