...
-- Recreate the function with proper exception handling for each step
...
DECLARE
  ...
  v_subject text;
BEGIN
    -- Block for inserting into public.users
    BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
        -- Ignore if the user already exists, but log it for debugging.
        INSERT INTO public.trigger_logs (message, error_message, metadata)
        VALUES ('User already exists in public.users, skipping insert.', SQLERRM, jsonb_build_object('user_id', NEW.id, 'email', NEW.email));
    END;

    -- Block for inserting into user_profiles
    BEGIN
    ...
    -- Handle role-specific logic, which will now run even if the user already existed.
