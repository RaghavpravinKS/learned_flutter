-- Step 1: Add the missing UNIQUE constraint to the user_profiles table.
ALTER TABLE public.user_profiles ADD CONSTRAINT user_profiles_user_id_key UNIQUE (user_id);

-- Step 2: Drop the existing trigger and function to ensure a clean re-application.
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Step 3: Recreate the function, which is now correctly supported by the schema.
...
BEGIN
    ...
    -- Create a user profile, ignoring conflicts on the now-unique user_id.
