...
-- This script will drop the entire public schema and recreate it.
...
-- Drop the trigger from the auth.users table first as it's in a different schema
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Drop the entire public schema and all its objects
...
-- The following content is from complete_schema_with_functions.sql
-- =============================================
...
CREATE TABLE public.teacher_verification (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  teacher_id uuid NOT NULL,
  verification_status public.teacher_status DEFAULT 'pending',
...
CREATE OR REPLACE FUNCTION public.enroll_student_with_payment(
    p_student_id uuid,
    p_classroom_id text,
...
BEGIN
    ...
    SELECT * INTO v_payment_plan_record FROM public.payment_plans WHERE id = p_payment_plan_id;
    IF NOT FOUND THEN
        RETURN jsonb_build_object('success', false, 'error', 'Payment plan not found');
    END IF;

    -- Check if already enrolled
    IF EXISTS (SELECT 1 FROM public.student_enrollments WHERE student_id = p_student_id AND classroom_id = p_classroom_id) THEN
    ...
    PERFORM public.log_audit_event(
        p_user_id := (SELECT user_id FROM public.students WHERE id = p_student_id),
...
CREATE OR REPLACE FUNCTION public.get_student_classrooms(p_student_id uuid)
RETURNS TABLE (
    classroom_id text,
    classroom_name text,
    subject text,
...
CREATE OR REPLACE FUNCTION public.create_teacher_by_admin(
    p_admin_id uuid,
    p_email text,
    p_first_name text,
    p_last_name text,
...
CREATE OR REPLACE FUNCTION public.update_expired_enrollments()
RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.renew_student_enrollment(
